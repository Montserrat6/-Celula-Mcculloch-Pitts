    ...
    def _tabla_de_verdad(self, n_bits, compuerta = "AND"):
        ...
        for j in range(len(matrix[0])):
            ...
            for i in range(len(matrix)):
                expresion.append(matrix[i][j])
            if compuerta == "AND":
                tt[str(expresion)] = True if expresion.count(1) ==n_bits else False
            if compuerta == "OR":
                tt[str(expresion)] = True if expresion.count(1) >= 1 else False
            if compuerta == "NOT":  
               tt[str(expresion)] = not expresion [0]  
        return tt
